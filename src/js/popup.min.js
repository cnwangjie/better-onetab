(function(){chrome.tabs.setZoom(1);document.title=chrome.i18n.getMessage("extName");$("body").attr("data-lan",chrome.i18n.getMessage("@@ui_locale"));$("#tips .title").html(chrome.i18n.getMessage("tipsTitle"));
...
}this._setLocalStorage(u);}}};f.on("click","li",function(w){w.preventDefault();var v=$(this),x=v.data("id"),u=v.closest("#hideList").length===1;if(x&&v.hasClass("hover")){v.removeClass("hover");
k.removeClass("extName-anim").attr("style","").empty();if(u){d.append(v);r.setRank(x);}else{n.append(v);}v.attr("data-leftClick",(new Date)*1);if(n.find("li").length===0){d.addClass("hideListIsNull");
}else{d.removeClass("hideListIsNull");}chrome.management.setEnabled(x,u,function(){});}m();});function c(){var u=$("[data-right]");if(u.length>0){u.removeAttr("data-right").removeClass("hover");
h.removeAttr("style");}$(".dinginess").removeClass("dinginess");k.removeClass("extName-anim").attr("style","").empty();}$(document).on("click",function(){c();
});var g={"1":18,"2":22,"3":26};f.on("mouseenter","li",function(){var C=$(this),u=C.attr("data-id");var w=$("[data-right]"),B=w.attr("data-id");if(u===B){return false;
}if(w.length>0){w.removeAttr("data-right").removeClass("hover");h.removeAttr("style");var A=C.closest("#showList").length>0?$("#hideList"):$("#showList");
A.removeClass("dinginess");}if(j){var D=C.offset();var x=D.left+50-10;var z=x+g[p];var v=k.html(C.attr("data-name")).outerWidth();if($("body").width()<v+z){x=D.left-v+10;
z=x-g[p];}if(e[u]){k.css("background-color",e[u].color);}k.css({top:D.top+15,left:x});}var y=C.closest(".list");clearTimeout(window["timer_disable_dinginess_"+y.attr("id")]);
window["timer_"+C.data("id")]=setTimeout(function(){if((new Date)*1<(C.attr("data-leftclick")*1+200)){return;}C.addClass("hover");if(j){y.addClass("dinginess");
k.addClass("extName-anim").css({left:z,opacity:1});}},100);}).on("mouseleave","li",function(){var u=$(this),v=u.closest(".list");if(u.attr("data-right")!=undefined){return false;
}if(j){window["timer_disable_dinginess_"+v.attr("id")]=setTimeout(function(){v.removeClass("dinginess");},300);k.removeClass("extName-anim").attr("style","").empty();
}u.removeClass("hover");clearTimeout(window["timer_"+u.data("id")]);});function l(){var u=localStorage.getItem("_lockList_");if(u){return JSON.parse(u);
...
}n.append(x);chrome.management.setEnabled(z,false,function(){});});chrome.browserAction.setBadgeText({text:""});var u=$("#showList li[locked]").removeAttr("locked");
setTimeout(function(){u.attr("locked","");},0);});$(document).on("mousedown","#hideList>li, #showList>li",function(x){var D=$(this),v=D.data("id"),u=D.attr("locked")!=undefined,A=D.data("optionurl"),z=D.data("homepageurl"),C=$("[data-right]");
...
}localStorage.setItem("_lockList_",JSON.stringify(w));m();break;case"option":if(v.attr("disabled")===undefined){chrome.tabs.create({url:u.data("optionurl")});
...
},0);I=t;}return{color:I,substantial:L};}var u=new Image();u.crossOrigin="Anonymous";u.src=x;u.onload=function(){var y=w(u);e[v]=y;};}})();