...
if(w){u.removeClass("hover");if(s){d.append(u);r.setRank(w);}else{n.append(u);}u.attr("data-leftClick",(new Date)*1);if(n.find("li").length===0){d.addClass("hideListIsNull");
}else{d.removeClass("hideListIsNull");}chrome.management.setEnabled(w,s,function(){});}m();});function c(){var s=$("[data-right]");if(s.length>0){s.removeAttr("data-right").removeClass("hover");
h.removeAttr("style");$(".dinginess").removeClass("dinginess");}k.removeClass("extName-anim").attr("style","").empty();}$("body").on("click",function(){c();
});var g={"1":14,"2":18,"3":24};f.on("mouseenter","li",function(){var B=$(this),s=B.attr("data-id");var v=$("[data-right]"),A=v.attr("data-id");if(s===A){return false;
}if(v.length>0){v.removeAttr("data-right").removeClass("hover");h.removeAttr("style");var z=B.closest("#showList").length>0?$("#hideList"):$("#showList");
z.removeClass("dinginess");}if(j){var C=B.offset();var w=C.left+50-10;var y=w+g[p];var u=k.html(B.attr("data-name")).outerWidth();if($("body").width()<u+y){w=C.left-u+10;
y=w-g[p];}if(e[s]){k.css("background-color",e[s].color);}k.css({top:C.top+15,left:w});}var x=B.closest(".list");clearTimeout(window["timer_disable_dinginess_"+x.attr("id")]);
window["timer_"+B.data("id")]=setTimeout(function(){if((new Date)*1<(B.attr("data-leftclick")*1+200)){return;}B.addClass("hover");if(j){x.addClass("dinginess");
k.addClass("extName-anim").css({left:y,opacity:1});}},100);}).on("mouseleave","li",function(){var s=$(this),u=s.closest(".list");if(s.attr("data-right")!=undefined){return false;
}if(j){window["timer_disable_dinginess_"+u.attr("id")]=setTimeout(function(){u.removeClass("dinginess");},300);k.removeClass("extName-anim").attr("style","").empty();
}s.removeClass("hover");clearTimeout(window["timer_"+s.data("id")]);});function l(){var s=localStorage.getItem("_lockList_");if(s){return JSON.parse(s);
}else{return null;}}$("body").on("dblclick",function(){var s=d.find("li");s.each(function(u,w){var v=$(w);var x=v.data("id");if(v.attr("locked")===""){return;
}n.append(v);chrome.management.setEnabled(x,false,function(){});});chrome.browserAction.setBadgeText({text:""});});$(document).on("mousedown","#hideList>li, #showList>li",function(w){var C=$(this),u=C.data("id"),s=C.attr("locked")!=undefined,z=C.data("optionurl"),y=C.data("homepageurl"),B=$("[data-right]");
