...
if(x&&v.hasClass("hover")){v.removeClass("hover");k.removeClass("extName-anim").attr("style","").empty();if(u){d.append(v);r.setRank(x);}else{n.append(v);
}v.attr("data-leftClick",(new Date)*1);if(n.find("li").length===0){d.addClass("hideListIsNull");}else{d.removeClass("hideListIsNull");}chrome.management.setEnabled(x,u,function(){});
}m();});function c(){var u=$("[data-right]");if(u.length>0){u.removeAttr("data-right").removeClass("hover");h.removeAttr("style");$(".dinginess").removeClass("dinginess");
...
if(u){return JSON.parse(u);}else{return null;}}$("body").on("dblclick",function(){var v=d.find("li");v.each(function(w,y){var x=$(y);var z=x.data("id");
if(x.attr("locked")===""){return;}n.append(x);chrome.management.setEnabled(z,false,function(){});});chrome.browserAction.setBadgeText({text:""});var u=$("#showList li[locked]").removeAttr("locked");
