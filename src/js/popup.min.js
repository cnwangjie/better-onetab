...
if(x.length===0){b.addClass("allListIsEmpty");}}c.html(p.join(""));setTimeout(function(){b.height(b.height()+1);},0);a();});}d();function a(){if(!localStorage.getItem("_switch_show_badge_")){var o=j();
var n=0;$("#showList li").each(function(p,q){var r=$(q).data("id");if(o[r]!=="1"){n++;}});if(n===0){chrome.browserAction.setBadgeText({text:""});}else{chrome.browserAction.setBadgeBackgroundColor({color:"#f44336"});
chrome.browserAction.setBadgeText({text:n+""});}}}var l={_key:"_rankList_",_getLocalStorage:function(){var o=localStorage.getItem(this._key),n=null;if(o){n=JSON.parse(o);
return n;}else{return{};}},_setLocalStorage:function(n){localStorage.setItem(this._key,JSON.stringify(n));},get:function(o){var n=this._getLocalStorage();
if(n&&n[o]){return parseInt(n[o]);}else{return 0;}},setRank:function(p){var n=this._getLocalStorage();if(n){var o=n[p];if(o){n[p]=parseInt(n[p])+1;}else{n[p]=1;
}this._setLocalStorage(n);}}};b.on("click","li",function(){var o=$(this),p=o.data("id"),n=o.closest("#hideList").length===1;if(p){if(n){e.append(o);l.setRank(p);
}else{c.append(o);}if(c.find("li").length===0){e.addClass("hideListIsNull");}else{e.removeClass("hideListIsNull");}chrome.management.setEnabled(p,n,function(){});
}a();});var g=$("#extName");var f=localStorage.getItem("_switch_show_extname_")!=="close";var h={"1":16,"2":20,"3":24};b.on("mouseenter","li",function(){var q=$(this);
if(f){var o=q.offset();var p=o.left+50-10;var r=p+h[k];var n=g.html(q.attr("data-name")).outerWidth();if($("body").width()<n+r){p=o.left-n+10;r=p-h[k];
}g.css({top:o.top+15,left:p});}window["timer_"+q.data("id")]=setTimeout(function(){q.addClass("hover");if(f){g.addClass("extName-anim").css({left:r,opacity:1});
