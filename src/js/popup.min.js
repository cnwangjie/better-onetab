...
function d(){var p=j();chrome.management.getAll(function(x){var q=[];var z=[];var r=[];for(var t=0;t<x.length;t++){var s=x[t];if(s.id===chrome.app.getDetails().id||s.type==="theme"){continue;
}var u="";if(s.icons){u=s.icons[s.icons.length-1].url;}var v="";if(p&&p[s.id]==1){v="locked";}var y="";if(!s.icons||s.icons.length==0){y="noicon";}var w='<li class="'+y+'" data-id="'+s.id+'" data-optionurl="'+s.optionsUrl+'" data-name="'+s.shortName+'" style="background-image:url('+u+')" '+v+"></li>";
if(f&&s.icons&&s.icons.length>0){(function(B,C){setTimeout(function(){n(B,C.id);},0);})(s.icons[0].url,s);}if(s.enabled===false){r.push(w);}else{if(v==="locked"){z.unshift(w);
}else{q.push(w);}}}if(!localStorage.getItem("_switch_rank_sort_")){q.sort(function(B,C){return m.get($(C).data("id"))-m.get($(B).data("id"));});r.sort(function(B,C){return m.get($(C).data("id"))-m.get($(B).data("id"));
});}var A=z.concat(q);e.html(A.join(""));if(r.length===0){e.addClass("hideListIsNull");if(A.length===0){b.addClass("allListIsEmpty");}}c.html(r.join(""));
